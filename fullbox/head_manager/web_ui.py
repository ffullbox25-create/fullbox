...
from fbs.services.racks import configure_fbs_rack
...
from fbs.services.wave_queue import (
    current_wave_queue,
...
from sklad.services.shared_location_catalog import ensure_shared_fbs_storage_cell
...
def _movement_report_response() -> HttpResponse:
    ...
    sheet = workbook.create_sheet(title="Отчет")
    ...
    header_cells = []
    for value in _MOVEMENT_REPORT_HEADERS:
    ...
    for row in rows:
        values = [_excel_safe_value(row.get(key, "")) for key in row_keys]
        if row.get("is_processing_source") and str(values[3] or "").strip():
            cell = WriteOnlyCell(sheet, value=values[3])
    ...
    def post(self, request, employee_id: int):
        with transaction.atomic():
            employee = get_object_or_404(
                Employee.objects.select_for_update(of=("self",)).select_related(
                    "user"
                ),
                pk=employee_id,
            )
...
def _head_manager_fbs_tote_journal(carts):
    free_states = {
        FbsToteBinding.STATE_UNBOUND,
        FbsToteBinding.STATE_FREE,
    }
    work_states = {
        FbsToteBinding.STATE_PICKING,
        FbsToteBinding.STATE_AT_CONTROL,
        FbsToteBinding.STATE_CHECKING,
        FbsToteBinding.STATE_UNKNOWN,
    }
    category_meta = {
        "work": ("В работе", "info", 0),
        "occupied": ("Занята", "warning", 1),
        "free": ("Свободна", "success", 2),
        "disabled": ("Отключена", "neutral", 3),
    }
    summary = {"work": 0, "occupied": 0, "free": 0, "disabled": 0}
    rows = []
    ...
    for cart in carts:
        try:
            ...
            binding = None
        state = binding.state if binding else FbsToteBinding.STATE_UNBOUND
        if not cart.is_active:
            ...
            category = "occupied"
        status_label, status_class, sort_order = category_meta[category]
        summary[category] += 1

        if binding and binding.zone_id:
            location_label = f"Зона: {binding.zone.name}"
            location_detail = binding.zone.barcode
        elif binding and binding.workstation_id:
            location_label = f"Стол: {binding.workstation.name}"
            location_detail = binding.workstation.barcode
        elif binding and binding.employee_id:
            employee_profile = getattr(binding.employee, "employee_profile", None)
            employee_name = (
                str(getattr(employee_profile, "full_name", "") or "").strip()
                or str(binding.employee.get_full_name() or "").strip()
                or binding.employee.get_username()
            )
            location_label = f"У сотрудника: {employee_name}"
            location_detail = binding.employee.get_username()
        elif state == FbsToteBinding.STATE_UNBOUND:
            location_label = "Общий пул"
            location_detail = "Место ещё не назначено"
        else:
            location_label = "Место не зафиксировано"
            location_detail = "Проверьте текущую привязку"

        assignment_label = "Нет активной работы"
        assignment_detail = "—"
        if binding and state not in free_states:
            if binding.pick_batch_id:
                assignment_label = f"Волна #{binding.pick_batch_id}"
                assignment_detail = binding.pick_batch.get_status_display()
            elif binding.controller_session_id:
                assignment_label = f"Контроль #{binding.controller_session_id}"
                controller = binding.controller_session.controller
                controller_profile = getattr(controller, "employee_profile", None)
                assignment_detail = (
                    str(getattr(controller_profile, "full_name", "") or "").strip()
                    or str(controller.get_full_name() or "").strip()
                    or controller.get_username()
                )
            else:
                assignment_label = "Занята без волны"
                assignment_detail = "Текущее действие указано в статусе"
        ...
        rows.append(
            {
                ...
                "state_label": binding.get_state_display() if binding else "Не привязана",
                "location_label": location_label,
                "location_detail": location_detail,
                "assignment_label": assignment_label,
                "assignment_detail": assignment_detail,
                "updated_at": binding.updated_at if binding else cart.updated_at,
                "sort_order": sort_order,
            }
        )

    rows.sort(key=lambda row: (row["sort_order"], row["cart"].name.lower(), row["cart"].id))
    ...
    def get_context_data(self, **kwargs):
        ...
        carts = list(
            FbsPickingCart.objects.select_related(
                ...
                "binding__employee__employee_profile",
                "binding__pick_batch",
                "binding__controller_session__controller__employee_profile",
            )
        )
        tote_rows, tote_summary = _head_manager_fbs_tote_journal(carts)
        agencies = list(
...
class HeadManagerFbsWavePolicyUpdateView(RoleRequiredMixin, View):
    allowed_roles = ("head_manager",)
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            ...
            if action == "reset":
                ...
                return redirect("/head-manager/fbs/settings/#wave-settings")
...
class HeadManagerFbsQueuePriorityUpdateView(RoleRequiredMixin, View):
    allowed_roles = ("head_manager",)
    ...
    def post(self, request):
        ...
        if action == "reorder_queue":
            try:
                ...
                messages.success(
                    request,
                    f"Положение волны #{batch_id} сохранено на сегодня.",
                )
            return redirect("/head-manager/fbs/settings/#queue-order")
        ...
        if action == "save_marketplace_priorities":
            ...
            return redirect("/head-manager/fbs/settings/#marketplace-priority")

        messages.error(request, "Неизвестная команда настройки очереди волн.")
        return redirect("/head-manager/fbs/settings/#queue-order")


_LOCATION_IMPORT_HEADERS = {
    "зона": "zone_code",
    "код места": "location_code",
    "название": "display_name",
    "вместимость": "capacity_containers",
    "видно в fbs": "is_fbs_visible",
    "участвует в отгрузках": "is_shipping",
}
_LOCATION_IMPORT_TRUE = {"1", "да", "yes", "true", "+"}
_LOCATION_IMPORT_FALSE = {"", "0", "нет", "no", "false", "-"}


def _warehouse_locations_import_rows(upload) -> list[dict]:
    if upload is None:
        raise ValidationError("Выберите файл Excel в формате .xlsx.")
    filename = str(getattr(upload, "name", "") or "").strip()
    if not filename.lower().endswith(".xlsx"):
        raise ValidationError("Поддерживается только файл Excel в формате .xlsx.")
    if int(getattr(upload, "size", 0) or 0) > 2 * 1024 * 1024:
        raise ValidationError("Файл мест не должен быть больше 2 МБ.")
    try:
        ...
        raw_headers = next(source_rows, None)
        if not raw_headers:
            raise ValidationError("Файл Excel пуст.")
        headers = [str(value or "").strip().casefold() for value in raw_headers]
        missing = [label for label in _LOCATION_IMPORT_HEADERS if label not in headers]
        if missing:
            raise ValidationError(
                "Не найдены обязательные столбцы: " + ", ".join(missing) + "."
            )
        indexes = {
            target: headers.index(label)
            for label, target in _LOCATION_IMPORT_HEADERS.items()
        }
        parsed: list[dict] = []
        for row_number, values in enumerate(source_rows, start=2):
            ...
            flags = {}
            for key, label in (
                ("is_fbs_visible", "Видно в FBS"),
                ("is_shipping", "Участвует в отгрузках"),
            ):
                raw_flag = str(value(key) or "").strip().casefold()
                if raw_flag not in _LOCATION_IMPORT_TRUE | _LOCATION_IMPORT_FALSE:
                    raise ValidationError(
                        f"Строка {row_number}: в столбце «{label}» укажите Да или Нет."
                    )
                flags[key] = raw_flag in _LOCATION_IMPORT_TRUE
            capacity_value = value("capacity_containers")
            try:
                capacity = int(capacity_value)
            except (TypeError, ValueError):
                raise ValidationError(
                    f"Строка {row_number}: вместимость должна быть целым числом."
                )
            parsed.append(
                {
                    "zone_code": str(value("zone_code") or "").strip(),
                    "location_code": str(value("location_code") or "").strip(),
        ...
        raise ValidationError(
            "Не удалось прочитать Excel. Скачайте шаблон и проверьте структуру файла."
        ) from exc
    finally:
...
def _warehouse_locations_template_response() -> HttpResponse:
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Места"
    sheet.append(["Зона", "Код места", "Название", "Вместимость", "Видно в FBS", "Участвует в отгрузках"])
    sheet.append(["PR", "PR-DOP-01", "Стол приёмки №1", 20, "Да", "Нет"])
    sheet.append(["OBR", "OBR-DOP-01", "Зона обработки №1", 20, "Нет", "Нет"])
    sheet.append(["OTG", "OTG-DOP-01", "Зона отгрузки №1", 20, "Да", "Да"])
    sheet.append(["OS", "OS-DOP-01", "Дополнительное место основного склада", 20, "Нет", "Нет"])
    sheet.append(["MR", "MR-DOP-01", "Дополнительное место между рядами", 20, "Нет", "Нет"])
    header_fill = PatternFill("solid", fgColor="F6B72B")
    for cell in sheet[1]:
        cell.font = Font(bold=True)
        cell.fill = header_fill
    widths = (14, 22, 40, 16, 18, 28)
    for index, width in enumerate(widths, start=1):
        sheet.column_dimensions[get_column_letter(index)].width = width
    sheet.freeze_panes = "A2"
    output = BytesIO()
    ...
    response["Content-Disposition"] = 'attachment; filename="warehouse_locations_template.xlsx"'
    return response


def _warehouse_location_qr_value(location):
    # PR labels carry the address itself, so handheld readers show plain text.
    # Legacy LOC:<warehouse>:<address> scans remain supported by the resolver.
    if location.zone_code == "PR":
        return location.location_code
    return f"LOC:{location.warehouse_code}:{location.location_code}"
...
class HeadManagerWarehouseLocationsView(RoleRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        if request.GET.get("download_template") == "1":
            return _warehouse_locations_template_response()
        return super().get(request, *args, **kwargs)

    def post(self, request, *args, **kwargs):
        try:
            if request.POST.get("action") == "import":
                imported = create_operational_locations(
                    _warehouse_locations_import_rows(request.FILES.get("locations_file"))
                )
            else:
                imported = None
                create_operational_location(
                    ...
                    allow_mixed_client_pallets=(
                        request.POST.get("allow_mixed_client_pallets") == "1"
                    ),
                    is_shipping=request.POST.get("is_shipping") == "1",
                )
            ...
            if imported is not None:
                messages.success(
                    request,
                    f"Импорт завершён: создано мест — {len(imported)}. Можно печатать QR.",
                )
            else:
    ...
    def get_context_data(self, **kwargs):
        ...
        rows = list(
            WarehouseLocation.objects.filter(
                warehouse_code="MSK",
                zone_code__in=tuple(OPERATIONAL_ZONE_KINDS),
                is_topology_visible=False,
            )
            .filter(
                Q(fbs_storage_cell__rack_cell__isnull=True)
                | Q(fbs_rack__isnull=False)
            )
            .distinct()
            .select_related("fbs_rack")
            .prefetch_related(
                Prefetch(
                    "fbs_rack__cells",
        ...
        for location in rows:
            ...
            location.qr_value = _warehouse_location_qr_value(location)
            rack = getattr(location, "fbs_rack", None)
            location.fbs_rack_config = rack
            location.fbs_rack_cells = list(rack.cells.all()) if rack else []
            location.fbs_rack_cell_count = sum(
                bool(cell.is_active) for cell in location.fbs_rack_cells
            )
        ...
        print_location_ids = {
            int(value)
            for value in self.request.GET.getlist("print_selected")[:300]
            if str(value).isdigit()
        }
        if print_location_ids:
            selected_locations = [
                location for location in rows if int(location.id) in print_location_ids
            ]
        selected_rack = None
        rack_print_cells = []
        print_rack_location_id = str(self.request.GET.get("print_rack") or "").strip()
        if print_rack_location_id.isdigit():
            rack_location = next(
                (
                    location
                    for location in rows
                    if int(location.id) == int(print_rack_location_id)
                ),
                None,
            )
            selected_rack = getattr(rack_location, "fbs_rack_config", None)
            if selected_rack is not None:
                rack_print_cells = [
                    cell
                    for cell in rack_location.fbs_rack_cells
                    if cell.is_active
                    and cell.storage_cell.is_active
                    and cell.storage_cell.location.is_active
                ]
                for cell in rack_print_cells:
                    cell.qr_value = _warehouse_location_qr_value(
                        cell.storage_cell.location
                    )
        location_zone_choices = [
            {"code": code, "label": OPERATIONAL_ZONE_LABELS[code]}
            for code in OPERATIONAL_ZONE_KINDS
        ]
        location_zone_groups = [
            {
                "code": choice["code"],
                "label": choice["label"],
                "locations": [
                    row for row in rows if row.zone_code == choice["code"]
                ],
            }
            for choice in location_zone_choices
        ]
        context.update(
            {
                "locations": rows,
                "location_zone_choices": location_zone_choices,
                "location_zone_groups": location_zone_groups,
                "selected_location": selected_location,
                "selected_locations": selected_locations,
                "selected_rack": selected_rack,
                "rack_print_cells": rack_print_cells,
                "sidebar_active": "settings",
                "summary": {
                    "total": len(rows),
                    "active": sum(row.is_active for row in rows),
                    "fbs": sum(row.is_fbs_visible for row in rows),
                    "shipping": sum(row.is_shipping for row in rows),
...
class HeadManagerFbsRackConfigureView(RoleRequiredMixin, View):
    allowed_roles = ("head_manager",)
    ...
    def post(self, request, location_id: int):
        try:
            rack = configure_fbs_rack(
                location_id=location_id,
                cell_count=int(request.POST.get("fbs_rack_cell_count") or 0),
            ...
            active_count = rack.cells.filter(is_active=True).count()
            messages.success(
                request,
                f"Для {rack.rack_code} настроено FBS-ячеек: {active_count}.",
            )
        return redirect("head-manager-warehouse-locations")
