...
def _warehouse_base_rows_for_planning(
    *,
    ...
    return [row for row in rows if _row_is_available_reachtruck_source(row)]


def _row_is_available_reachtruck_source(row) -> bool:
    ...
    if active_context_type == "reachtruck_free" and active_context_status in {
        "created",
        "planned",
        "in_progress",
        "partial",
        ...
        return False
    if _as_int(_row_value(row, "active_operation_id", 0)) > 0:
        return False
    return True
...
def _assign_stock_boxes_to_source_patterns(
    *,
    ...
    for box in _all_stock_boxes_for_pallet(
        ...
        if not box_code or box_key in seen_boxes or box_key in claimed_box_keys:
            continue
...
def _shipping_reserve_planning_rows(*, agency_id: int | None) -> list[dict]:
    rows = _warehouse_base_rows_for_planning(agency_id=agency_id)
...
def _shipping_reserve_box_plan_for_items(*, items: list, agency_id: int | None) -> dict:
    ...
    base_rows = _shipping_reserve_planning_rows(agency_id=agency_id)
...
def _shipping_item_box_demands(item) -> list[dict]:
    ...
    if box_count <= 0 or box_qty <= 0 or qty_required <= 0:
        return []
    if box_count * box_qty != qty_required:
        return []
    requested_article = str(getattr(item, "sku_code", "") or "").strip()
    requested_goods_type = _normalize_goods_type(getattr(item, "goods_type", ""))
...
def _build_shipping_box_demand_candidates(
    *,
    ...
    for row in base_rows:
        ...
        pallet_code = str(_row_value(row, "pallet_code", "") or "").strip()
        if box_code and box_code.lower() in claimed_box_keys:
            continue
        if box_code:
    ...
    for demand_idx, demand in enumerate(demands):
        ...
        for box_key, raw_row_refs in sorted(box_rows_by_code.items()):
            ...
            if not box_code or box_key in claimed_box_keys:
                continue
        ...
        for pallet_code in sorted(pallet_meta.keys()):
            ...
            for row in _matching_stock_boxes_for_patterns(
                ...
                box_key = box_code.lower()
                if box_key in claimed_box_keys:
                    continue
                row_refs = []
...
def _plan_shipping_box_demands(
    *,
    ...
    for item in items:
        item_demands = _shipping_item_box_demands(item)
        if not item_demands:
            continue
        demands.extend(item_demands)
        planned_item_ids.add(int(item.id))
    if not demands:
        return None
