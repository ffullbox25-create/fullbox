...
def _otg_selector_sets(payload: dict) -> tuple[set[str], set[str], set[str], int]:
    ...
    if barcode_values:
        ...
        agency_id = _parse_int_value(payload.get("agency_id"))
        if agency_id:
...
def _stock_box_matches_requested_pattern(row: dict, pattern: dict) -> bool:
    ...
    if pattern_barcode_qty:
        return row_barcode_qty == pattern_barcode_qty
    requested_barcodes = {
        str(value).strip()
    ...
    if requested_barcodes:
        row_barcodes = {barcode for barcode in row_barcode_qty.keys() if barcode}
...
def _pallet_box_plan(
    ...
    if move_mode == MOVE_MODE_PALLET_FULL:
        ...
        if selection_mode == BOX_SELECTION_PATTERN_MATCHING:
            ...
            barcode_values, sku_values, goods_type_values, _requested_qty = _otg_selector_sets(payload)
            matching_codes = {
