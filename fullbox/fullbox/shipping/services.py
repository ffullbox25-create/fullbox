...
def reserve_order(
    ...
    *,
    ...
    from reachtruck.services.move_requests import _shipping_reserve_box_plan_for_items

    reserve_box_plan = _shipping_reserve_box_plan_for_items(
        items=items,
        agency_id=order.agency_id,
        exclude_shipping_order_id=order.number,
    )
    if reserve_box_plan.get("failed"):
        raise ValidationError(
            "Не удалось подобрать короба под коробочную схему отгрузки из фактически доступных полных коробов."
        )
    warehouse_items: list[dict] = []
    pattern_item_ids = {
        int(item_id or 0)
        for item_id in (reserve_box_plan.get("pattern_item_ids") or set())
    ...
    for item in items:
        ...
        if qty <= 0:
            ...
            continue
        partial_box_split = extract_partial_box_split(item.comment)
        reserve_as_pool = True
        box_codes = []
        warehouse_items.append(
            {
                ...
                "qty": qty,
                "box_codes": box_codes,
                "reserve_pool": reserve_as_pool,
                "reserve_box_plan_checked": int(item.id or 0) in pattern_item_ids,
                "allow_partial_box_reserve": bool(partial_box_split),
            }
        )
...
def shipping_pick_readiness(order: ShippingOrder) -> dict[str, str | bool]:
    ...
    from .selectors import shipping_otg_tasks_progress
    ...
    if expected_boxes > 0:
        delivered_box_codes = {
            ...
            for code in WarehouseStockSnapshot.objects.filter(
                agency=order.agency,
                is_archived=False,
                last_event__stock_context_type="shipping",
                last_event__stock_context_id=str(order.number or "").strip(),
        ...
        if len(delivered_box_codes) >= expected_boxes:
            return {
                "can_pick": False,
    ...
    *,
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and not parse_errors:
            client_returns_submitted_to_draft = (
                edit_order is not None
                and scope == "client"
                and edit_order.status == ShippingOrder.STATUS_SUBMITTED
            )
                ...
                with transaction.atomic():
                    ...
                    if edit_order is not None:
                        ...
                        shipping_views._log_update(
                            ...
                            request,
                            (
                                "Заявка возвращена в подготовку и отредактирована клиентом"
                                if client_returns_submitted_to_draft
                            ...
                            extra={
                                ...
                                "edit_mode": True,
                                "returned_to_draft": client_returns_submitted_to_draft,
                            },
                        )
                    else:
...
def handle_shipping_packing_request(*, request, pk: int):
    ...
    if request.method == "GET" and packing_state["packing_summary"] and packing_state["delivered_boxes"]:
        return HttpResponseForbidden("Доступ запрещен")
...
def _ship_reserved_order_without_trip(order: ShippingOrder) -> None:
    ...
    for snapshot in snapshots:
        shipped_qty = WarehouseWritePathService._shipping_snapshot_flow_qty(snapshot)
        if shipped_qty <= 0:
            continue
