...
def locally_marked_item_ids(items) -> set[int]:
    """Items whose KIZ was scanned and stored although the marketplace refuses it.

    A SKU flagged for Chestny Znak makes the marking mandatory inside Fullbox even
    when the marketplace answers "not required".  In that case no metadata transfer
    is ever created -- a transfer exists only to hand the code over to the
    marketplace -- so a readiness gate that waits for a *resolved transfer* can
    never be satisfied and the shipment hangs forever with the goods already
    scanned and packed.

    The evidence for the internal obligation is the scanned code itself, kept on
    the traceability row.  Only items the marketplace refuses are considered here:
    whenever a transfer is allowed, the transfer stays the single source of truth.
    """

    candidates = [
        item
        for item in items
        if not metadata_requirements(item).marketplace_marking_required
        and not marketplace_marking_transfer_allowed(item)
    ]
    if not candidates:
        return set()
    return set(
        FbsOrderTraceability.objects.filter(
            allocation__order_item_id__in=[item.id for item in candidates],
        )
        .exclude(marking_code="")
        .exclude(marking_code__isnull=True)
        .values_list("allocation__order_item_id", flat=True)
    )


def legacy_unmarked_item_ids(
    items: list[FbsOrderItem], *, pick_task_ids: list[int]
) -> set[int]:
    """Resolve no-code acknowledgements for the entire current picked item.

    This only replaces the synthetic requirement for a missing KIZ transfer.
    Callers must still validate any actual metadata transfer, including rejects.
    Never borrow a confirmation from another allocation or an earlier wave.
    """
    task_ids = {task_id for task_id in pick_task_ids if task_id is not None}
    candidates = {
        item.id: int(item.quantity)
        for item in items
        if metadata_requirements(item).marking_required
        and not metadata_requirements(item).marketplace_marking_required
    }
    if not candidates or not task_ids:
        return set()
    acknowledgement = FbsPickScanEvent.objects.filter(
        allocation_id=OuterRef("pk"),
        task_id=OuterRef("pick_task_id"),
        batch_id=OuterRef("pick_task__batch_id"),
        stage=FbsPickScanEvent.STAGE_VERIFY_MARKING,
        result=FbsPickScanEvent.RESULT_SUCCESS,
        scan_value="",
        expected_value=LEGACY_MARKING_ABSENCE_EXPECTED_VALUE,
        quantity_after=OuterRef("qty_picked"),
    )
    allocations = (
        FbsOrderStockAllocation.objects.filter(
            order_item_id__in=candidates,
        ...
        .annotate(legacy_acknowledged=Exists(acknowledgement))
        .values(
            "order_item_id", "status", "qty_reserved", "qty_picked",
            "balance__marking_code", "traceability__id",
            "traceability__marking_code", "traceability__qty",
            "traceability__status", "legacy_acknowledged",
        )
    )
    confirmed_qty: dict[int, int] = {}
    invalid_item_ids: set[int] = set()
    for allocation in allocations:
        item_id = allocation["order_item_id"]
        qty = allocation["qty_picked"]
        if (
            allocation["status"] != FbsOrderStockAllocation.STATUS_PICKED
            or qty <= 0
            or allocation["qty_reserved"] != qty
            or not allocation["legacy_acknowledged"]
            or str(allocation["balance__marking_code"] or "").strip()
            or not allocation["traceability__id"]
            or str(allocation["traceability__marking_code"] or "").strip()
            or allocation["traceability__qty"] != qty
            or allocation["traceability__status"] != FbsOrderTraceability.STATUS_PICKED
        ):
            invalid_item_ids.add(item_id)
        else:
            confirmed_qty[item_id] = confirmed_qty.get(item_id, 0) + qty
    return {
        item_id for item_id, quantity in candidates.items()
        if item_id not in invalid_item_ids
        and quantity > 0 and confirmed_qty.get(item_id, 0) == quantity
    }
