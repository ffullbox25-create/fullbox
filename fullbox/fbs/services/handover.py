...
from .shipment_policy import (
    TOTE_SHIPMENT_PREFIX, assert_shipment_pick_batch, is_tote_shipment,
    order_pick_batch_id,
)


# These are monotonic downstream WB facts: an order that reached pickup or sale
# has necessarily passed marketplace sorting/acceptance already.
WB_ACCEPTED_STATUSES = {"sorted", "ready_for_pickup", "sold"}
...
HANDOVER_CLOSED_ORDER_STATUSES = {
...
def _assert_order_ready_for_handover_close(order: FbsOrder) -> None:
    """Accept locally ready orders and monotonic post-handover terminal states."""
    if order.internal_status in HANDOVER_CLOSED_ORDER_STATUSES:
        return
    status_label = str(order.get_internal_status_display() or order.internal_status)
    raise FbsHandoverError(
        f"Заказ {order.external_order_id} не готов к передаче: "
        f"статус «{status_label}». Завершите проверку заказа; "
        "если заказ отменен — исключите его и выполните возврат товара."
    )
...
def handover_composition_readiness(
    ...
    current_pick_task_ids = list(
        FbsControllerToteOrder.objects.filter(check_tote__handover_batch=batch)
        .exclude(status=FbsControllerToteOrder.STATUS_REMOVED)
        .values_list("pick_tote__pick_batch__tasks__id", flat=True)
        .distinct()
    )
    readiness_items = [
        item for assignment in assignments for item in assignment.order.items.all()
    ]
    legacy_item_ids = legacy_unmarked_item_ids(
        readiness_items,
        pick_task_ids=current_pick_task_ids,
    )
    # Marking the marketplace refuses to accept is proven by the scanned code
    # itself; waiting for a transfer that is never created would hang forever.
    locally_marked_ids = locally_marked_item_ids(readiness_items)
...
def tote_handover_compatibility_key(order, *, pick_batch_id: int) -> str:
    if order.profile.marketplace == FbsIntegrationProfile.MARKETPLACE_WB:
        base = wb_handover_compatibility_key(order, pick_batch_id=pick_batch_id)
    else:
        base = (
            f"warehouse:{order.profile.external_warehouse_id}:marketplace:"
            f"{order.profile.marketplace}:pick-batch:{pick_batch_id}"
        )
    key = f"{TOTE_SHIPMENT_PREFIX}{base}"
    if len(key) > FbsHandoverBatch._meta.get_field("compatibility_key").max_length:
        raise FbsHandoverError("Параметры отгрузки превышают допустимую длину.")
    return key


def _validate_assignment_tote(existing, *, order, pick_batch_id, check_tote):
    # Grandfather already assigned legacy orders, including existing mixed carts.
    if is_tote_shipment(existing.batch):
        wave_id = order_pick_batch_id(order.id, requested_id=pick_batch_id)
        assert_shipment_pick_batch(existing.batch, wave_id)
        _assert_check_tote_wave(check_tote, wave_id)


def _assert_check_tote_wave(check_tote, wave_id):
    if check_tote is not None and check_tote.pick_totes.exclude(
        pick_batch_id=wave_id
    ).exists():
        raise FbsHandoverError(
            "В этом потоке уже находится другая тара. Откройте отдельный поток отгрузки."
        )


def _shipment_for_pick_wave(*, order, wave_id, compatibility_key, lock_nowait=False):
    """Caller locks the profile, serializing first assignment within a cabinet.

    Finish an already assigned wave in its original shipment, without moving
    orders or splitting legacy supplies at rollout. Unassigned waves can only
    find versioned, wave-specific keys, never an old workstation-wide supply.
    """
    wave_order_ids = FbsPickTask.objects.filter(batch_id=wave_id).exclude(
        status=FbsPickTask.STATUS_CANCELED
    ).values("order_id")
    batch_ids = set(FbsHandoverOrderAssignment.objects.filter(
        order_id__in=wave_order_ids
    ).exclude(status=FbsHandoverOrderAssignment.STATUS_CANCELED).values_list("batch_id", flat=True))
    if len(batch_ids) > 1:
        raise FbsHandoverError(
            "Заказы этой тары уже находятся в разных отгрузках. "
            "Нужна физическая сверка оператором FBS; автоматически заказы не переносим."
        )
    candidates = FbsHandoverBatch.objects.select_for_update(nowait=lock_nowait)
    if batch_ids:
        batch = candidates.get(pk=next(iter(batch_ids)))
    else:
        # Include closed matches so a retry cannot create a second shipment for
        # a cart whose shipment was already delivered. A missing-WB recovery
        # creates an open replacement explicitly and retains the owner key.
        batch = candidates.filter(
            profile=order.profile, compatibility_key=compatibility_key,
        ).order_by("-id").first()
    if batch is None:
        return None
    assert_shipment_pick_batch(batch, wave_id)
    if batch.profile_id != order.profile_id or (
        is_tote_shipment(batch) and batch.compatibility_key != compatibility_key
    ):
        raise FbsHandoverError(
            "В таре несовместимые кабинеты или направления доставки. "
            "Передайте тару оператору FBS для разделения; в одну поставку их добавлять нельзя."
        )
    if batch.status != FbsHandoverBatch.STATUS_OPEN or batch.marketplace_state not in (
        FbsHandoverBatch.MARKETPLACE_DRAFT, FbsHandoverBatch.MARKETPLACE_CREATING,
        FbsHandoverBatch.MARKETPLACE_OPEN,
    ):
        raise FbsHandoverError("Отгрузка этой тары уже закрыта для новых заказов.")
    if batch.order_assignments.count() >= 1000:
        raise FbsHandoverError(
            "Достигнут лимит 1000 заказов. Передайте тару оператору для физического разделения."
        )
    return batch
...
def _ensure_wb_order_handover_assignment(
    *,
    ...
    if existing is not None:
        _validate_assignment_tote(existing, order=order, pick_batch_id=pick_batch_id, check_tote=check_tote)
    ...
    wave_id = order_pick_batch_id(order.id, requested_id=pick_batch_id)
    compatibility_key = tote_handover_compatibility_key(order, pick_batch_id=wave_id)
    batch = _shipment_for_pick_wave(
        order=order, wave_id=wave_id, compatibility_key=compatibility_key,
        lock_nowait=lock_nowait,
    )
    if batch is None or is_tote_shipment(batch):
        _assert_check_tote_wave(check_tote, wave_id)
    if batch is None:
...
def create_handover_batch(
    *,
    ...
    batch = FbsHandoverBatch.objects.create(
        ...
        compatibility_key=f"{TOTE_SHIPMENT_PREFIX}unbound:{uuid4().hex}",
        marketplace_state=(
            FbsHandoverBatch.MARKETPLACE_OPEN
            if external_supply_id
...
def add_order_to_handover_box(
    *,
    ...
    # Manual box scanning obeys the same boundary. Serialize first-owner
    # binding across different transport boxes belonging to this shipment.
    if is_tote_shipment(box.batch):
        FbsIntegrationProfile.objects.select_for_update().get(pk=order.profile_id)
        batch = FbsHandoverBatch.objects.select_for_update().get(pk=box.batch_id)
        wave_id = order_pick_batch_id(order.id)
        order.profile = box.batch.profile
        key = tote_handover_compatibility_key(order, pick_batch_id=wave_id)
        if batch.compatibility_key.startswith(f"{TOTE_SHIPMENT_PREFIX}unbound:"):
            owner = _shipment_for_pick_wave(order=order, wave_id=wave_id, compatibility_key=key)
            if owner is not None and owner.id != batch.id:
                raise FbsHandoverError("У этой тары уже есть отгрузка. Используйте ее, не создавайте вторую.")
            batch.compatibility_key = key
            batch.save(update_fields=["compatibility_key", "updated_at"])
        assert_shipment_pick_batch(batch, wave_id)
        if batch.compatibility_key != key:
            raise FbsHandoverError("Заказ несовместим с направлением доставки этой тары.")
    if box.batch.profile.marketplace == FbsIntegrationProfile.MARKETPLACE_WB:
...
def close_handover_box(*, box_id: int) -> FbsHandoverBox:
    ...
    for link in orders:
        _assert_order_ready_for_handover_close(link.order)
        if not _required_metadata_confirmed(
            link.order,
            handover_batch=box.batch,
        ):
            raise FbsHandoverError(
                f"Заказ {link.order.external_order_id}: не все обязательные "
                "данные подтверждены маркетплейсом."
            )
    warehouse_wb = (
        box.batch.profile.marketplace == FbsIntegrationProfile.MARKETPLACE_WB
        and not wb_handover_uses_marketplace_boxes(box.batch)
...
def _assert_verified_controller_auto_dispatch_preconditions(
    batch: FbsHandoverBatch,
    *,
    check_tote_id: int | None = None,
) -> None:
    """Require closed controller evidence before automatic delivery handoff."""
    if batch.profile.marketplace not in {
        FbsIntegrationProfile.MARKETPLACE_OZON,
        FbsIntegrationProfile.MARKETPLACE_WB,
    }:
        raise FbsHandoverError(
            "Автоматическая передача доступна только для Ozon и Wildberries."
        )
    if _has_handover_verification_override(batch):
        raise FbsHandoverError(
            "Автоматическая передача недоступна при служебном разрешении."
        )
    check_totes = FbsControllerCheckTote.objects.select_for_update(
        of=("self",)
    ).filter(handover_batch=batch)
    if check_tote_id is not None:
        check_totes = check_totes.filter(pk=check_tote_id)
    check_tote = check_totes.first()
    if check_tote_id is None and check_totes.count() != 1:
        raise FbsHandoverError(
            "Не удалось однозначно определить закрытую тару контроля."
        )
    if (
        check_tote is None
        ...
        raise FbsHandoverError(
            "Автоматическая передача возможна только после полной проверки контролером."
        )
    ...
    if FbsPickRestockRequest.objects.filter(
        ...
        raise FbsHandoverError(
            "Есть незавершенный возврат или отмененный заказ. Автоматическая передача остановлена."
        )
    ...
    if not assignments or any(
        ...
        raise FbsHandoverError(
            "Не все заказы подтверждены в текущей поставке."
        )
    ...
    if {link.order_id for link in links} != assignment_order_ids:
        raise FbsHandoverError(
            "Состав заказов не совпадает с подтвержденным составом поставки."
        )
    for link in links:
        if order_is_client_canceled_by_marketplace(link.order):
            raise FbsHandoverError(
                f"Заказ {link.order.external_order_id} отменен маркетплейсом. "
                "Автоматическая передача остановлена."
            )
        if (
            link.verified_at is None
            ...
            raise FbsHandoverError(
                "Не все официальные этикетки заказов подтверждены сканированием."
            )
    ...
    if packed_order_ids != assignment_order_ids:
        raise FbsHandoverError(
            "Не все заказы завершили контроль состава."
        )
    ...
    if not active_boxes.exists() or active_boxes.exclude(
        status=required_box_status
    ).exists():
        raise FbsHandoverError("Не все короба безопасно закрыты и проверены.")
    assert_handover_composition_ready(batch)
...
def dispatch_handover_batch(
    *,
    ...
    verified_controller_auto: bool = False,
    controller_check_tote_id: int | None = None,
    ...
    verified_controller_auto = bool(
        verified_controller_auto or verified_ozon_auto
    )
    dispatch_confirmed_by_print = supply_label_print_job_id is not None
    if verified_controller_auto and dispatch_confirmed_by_print:
        raise FbsHandoverError(
            "Нельзя одновременно подтверждать автоматическую передачу и печать поставки."
        )
    ...
    if not dispatch_confirmed_by_print and not verified_controller_auto:
        raise FbsHandoverError(
            "Сначала распечатайте ШК поставки. Статус «В пути» "
            "установится автоматически после подтверждения печати."
        )
    if verified_controller_auto:
        _assert_verified_controller_auto_dispatch_preconditions(
            batch,
            check_tote_id=controller_check_tote_id,
        )
    if _has_handover_verification_override(batch):
        _assert_handover_verification_override_preconditions(batch)
    if FbsControllerCheckTote.objects.filter(handover_batch=batch).exists():
    ...
    if batch.profile.marketplace == FbsIntegrationProfile.MARKETPLACE_WB:
        ...
        if (
            not verified_controller_auto
            and (not batch.supply_qr_code or not batch.supply_label_file)
        ):
    ...
    if (
        ...
        and not verified_controller_auto
    ):
    ...
    log_order_bulk_transition(
        ...
        user=actor,
        source=(
            "dispatch_handover_batch:verified_controller_auto"
            if verified_controller_auto
            else (
...
@transaction.atomic
def request_wb_handover_delivery(
    *,
    batch_id: int,
    requested_by=None,
    ...
    for box in boxes:
        ...
        for link in links:
            ...
            if not _required_metadata_confirmed(
                ...
                raise FbsHandoverError(
                    f"Заказ {link.order.external_order_id}: не все обязательные "
                    "КИЗы и сроки годности подтверждены маркетплейсом."
                )
    ...
    if controller_auto_dispatch_check_tote_id is not None:
        schedule_kwargs["controller_auto_dispatch_check_tote_id"] = (
            controller_auto_dispatch_check_tote_id
        )
    return schedule_wb_handover_delivery(**schedule_kwargs)
