...
LABEL_SIZES = [
    ...
    {
        "key": "shipping_packing_slip",
        "title": "Упаковочный лист",
        "width_mm": 75,
        "height_mm": 120,
        "preview_scale": 0.95,
        "description": "Упаковочный лист паллеты отгрузки (75x120).",
    },
]

LABEL_SIZE_MAP = {item["key"]: item for item in LABEL_SIZES}
...
def _build_label_template(
    ...
    *,
    ...
    size_entry = LABEL_SIZE_MAP[size_key]
    return {
        "key": key,
        "size_key": size_key,
        ...
        "mode": mode,
        "category": category,
        "title": size_entry.get("title") or "Этикетка",
        "template_title": template_title,
        "button_title": button_title,
        "preview_title": preview_title,
        "width_mm": size_entry.get("width_mm", 58),
        "height_mm": size_entry.get("height_mm", 40),
        "design_width_mm": size_entry.get("width_mm", 58),
        "design_height_mm": size_entry.get("height_mm", 40),
        "preview_scale": size_entry.get("preview_scale", 1),
        "description": description,
    }


LABEL_TEMPLATES = [
    _build_label_template(
        "item",
        size_key="item",
        layout_key="item",
        mode="no-cz",
        category="product",
        button_title="58/40 · товар · основной",
        preview_title="Товар · 58x40 мм · основной",
        template_title="Основной",
        description="Основной шаблон товара 58x40.",
    ),
    _build_label_template(
        "item_alt",
        size_key="item",
        ...
        mode="no-cz",
        category="product",
        button_title="58/40 · товар · доп.",
        preview_title="Товар · 58x40 мм · доп. шаблон",
        template_title="Доп. шаблон",
        description="Дополнительный шаблон товара 58x40.",
    ),
    _build_label_template(
        "item_cz",
        size_key="item_cz",
        ...
        mode="cz",
        category="product",
        button_title="58/40 · товар ЧЗ · основной",
        preview_title="Товар ЧЗ · 58x40 мм · основной",
        template_title="Основной",
        description="Основной шаблон товара ЧЗ 58x40.",
    ),
    _build_label_template(
        "item_cz_alt",
        size_key="item_cz",
        ...
        mode="cz",
        category="product",
        button_title="58/40 · товар ЧЗ · доп.",
        preview_title="Товар ЧЗ · 58x40 мм · доп. шаблон",
        template_title="Доп. шаблон",
        description="Дополнительный шаблон товара ЧЗ 58x40.",
    ),
    _build_label_template(
        "item_5860",
        size_key="item_5860",
        ...
        mode="no-cz",
        category="product",
        button_title="58/60 · товар · основной",
        preview_title="Товар · 58x60 мм · основной",
        template_title="Основной",
        description="Основной шаблон товара 58x60.",
    ),
    _build_label_template(
        "item_5860_alt",
        size_key="item_5860",
        ...
        mode="no-cz",
        category="product",
        button_title="58/60 · товар · доп.",
        preview_title="Товар · 58x60 мм · доп. шаблон",
        template_title="Доп. шаблон",
        description="Дополнительный шаблон товара 58x60.",
    ),
    _build_label_template(
        "item_75120",
        size_key="item_75120",
        ...
        mode="no-cz",
        category="product",
        button_title="75/120 · товар · основной",
        preview_title="Товар · 75x120 мм · основной",
        template_title="Основной",
        description="Основной шаблон товара 75x120.",
    ),
    _build_label_template(
        "item_75120_alt",
        size_key="item_75120",
        ...
        mode="no-cz",
        category="product",
        button_title="75/120 · товар · доп.",
        preview_title="Товар · 75x120 мм · доп. шаблон",
        template_title="Доп. шаблон",
        description="Дополнительный шаблон товара 75x120.",
    ),
    _build_label_template(
        "box",
        size_key="box",
        ...
        mode="no-cz",
        category="storage",
        button_title="Короб 58x60",
        preview_title="Короб · 58x60 мм",
        template_title="Основной",
        description="Шаблон этикетки короба 58x60.",
    ),
    _build_label_template(
        "pallet",
        size_key="pallet",
        ...
        mode="no-cz",
        category="storage",
        button_title="Паллета 58x60",
        preview_title="Паллета · 58x60 мм",
        template_title="Основной",
        description="Шаблон этикетки паллеты 58x60.",
    ),
    _build_label_template(
        "shipping_packing_slip",
        size_key="shipping_packing_slip",
        layout_key="shipping_packing_slip",
        mode="no-cz",
        category="shipping",
        button_title="Упаковочный лист 75x120",
        preview_title="Упаковочный лист · 75x120 мм",
        template_title="Упаковочный лист",
        description="Шаблон упаковочного листа паллеты отгрузки 75x120.",
    ),
]
...
PRODUCT_LABEL_TEMPLATE_PAIRS = (
    ("item", "item_cz"),
    ("item_alt", "item_cz_alt"),
)
LABEL_SIZE_KEYS = [item["key"] for item in LABEL_SIZES]
LABEL_TEMPLATE_KEYS = [item["key"] for item in LABEL_TEMPLATES]
LABEL_TEMPLATE_MAP = {item["key"]: item for item in LABEL_TEMPLATES}
LABEL_DIMENSION_MIN_MM = 20
LABEL_DIMENSION_MAX_MM = 200
PROCESSING_PARAM_TEMPLATE_BINDINGS_DEFAULT = {
    "item": "item",
    "item_5860": "item_5860",
    "item_75120": "item_75120",
    "item_cz": "item_cz",
}
PROCESSING_PARAM_TEMPLATE_KEYS = tuple(PROCESSING_PARAM_TEMPLATE_BINDINGS_DEFAULT.keys())
...
def agent_printer_details_from_meta(meta: dict | None) -> list[dict]:
...
def agent_printer_names_from_meta(meta: dict | None) -> list[str]:
    if not isinstance(meta, dict):
        return []
    current = _normalize_printer_names(meta.get("printers"))
    if current:
        return current
    detail_names = _normalize_printer_names(
        [
            str(item.get("name") or "").strip()
            for item in agent_printer_details_from_meta(meta)
            if str(item.get("name") or "").strip()
        ]
    )
    if detail_names:
        return detail_names
    return _normalize_printer_names(meta.get("last_known_printers"))
...
def _load_agent_printers_data() -> tuple[list[str], dict]:
    ...
    for agent in agents:
        ...
        contributed = False
        for printer in agent_printer_names_from_meta(meta):
            key = printer.lower()
            if key in seen:
                continue
            seen.add(key)
        ...
        if contributed and latest_agent is None and agent.last_seen:
            latest_agent = agent
...
def _preferred_print_agent():
    ...
    if preferred:
        preferred_agent = base_qs.filter(
            models.Q(name=preferred) | models.Q(host=preferred) | models.Q(agent_id=preferred)
        ).first()
        if preferred_agent and agent_printer_names_from_meta(preferred_agent.meta):
            return preferred_agent
    online_candidates = list(base_qs.filter(last_seen__gte=online_since)[:10])
    for agent in online_candidates:
        if agent_printer_names_from_meta(agent.meta):
            return agent
    if preferred_agent:
        return preferred_agent
...
def _print_agent_targets(agent) -> list[str]:
    if agent is None:
        return []
    targets: list[str] = []
    for candidate in (getattr(agent, "agent_id", ""), getattr(agent, "name", ""), getattr(agent, "host", "")):
...
def build_print_status_snapshot() -> dict:
    ...
    raw_details = agent_printer_details_from_meta(agent_meta)
    ...
    print_jobs_qs = ProcessingPrintJob.objects.all()
    agent_targets = _print_agent_targets(agent)
    if agent_targets:
        print_jobs_qs = print_jobs_qs.filter(
            models.Q(agent="") | models.Q(agent__in=agent_targets)
        )
    pending_qs = print_jobs_qs.filter(status=ProcessingPrintJob.STATUS_PENDING).order_by("created_at")
    printing_qs = print_jobs_qs.filter(status=ProcessingPrintJob.STATUS_PRINTING).order_by("created_at")
    failed_qs = print_jobs_qs.filter(status=ProcessingPrintJob.STATUS_FAILED).order_by("-updated_at")
    ...
    def _format_job(job: ProcessingPrintJob) -> str:
        ...
        copies_suffix = f" x{job.copies_count}" if int(job.copies_count or 1) > 1 else ""
        if label:
            return f"#{job.id}{copies_suffix} · {target} · {label}"
        return f"#{job.id}{copies_suffix} · {target}"
    ...
    pending_count = ProcessingPrintJob.total_copies(pending_qs)
    printing_count = ProcessingPrintJob.total_copies(printing_qs)
    failed_count = ProcessingPrintJob.total_copies(failed_qs)
    stuck_count = ProcessingPrintJob.total_copies(stuck_qs)
...
def refresh_print_agent_printers() -> tuple[dict, str]:
    ...
    normalized_printers = _normalize_printer_names(printer_names)
    if normalized_printers:
        meta["printers"] = normalized_printers
        meta["last_known_printers"] = normalized_printers
        meta["last_known_printers_at"] = timezone.now().isoformat()
    if normalized_rows:
...
def _write_json_file(path: Path, payload: dict) -> None:
    ...
    _load_print_agent_status_cached.cache_clear()
...
def _normalize_scanner_block(data: dict) -> dict:
    ...
    if port:
        com_match = re.fullmatch(r"com0*(\d+)", port, flags=re.IGNORECASE)
        if com_match:
            port = f"COM{int(com_match.group(1))}"
...
def get_label_template(template_key: str) -> dict:
    key = str(template_key or "").strip()
    return dict(LABEL_TEMPLATE_MAP.get(key) or {})


def clean_label_dimensions(data: dict | None) -> dict:
    if not isinstance(data, dict):
        return {}
    ...
    for field in ("width_mm", "height_mm"):
        ...
        if parsed != rounded:
            continue
        if not LABEL_DIMENSION_MIN_MM <= rounded <= LABEL_DIMENSION_MAX_MM:
            continue
        cleaned[field] = rounded
    return cleaned


def get_effective_label_template(template_key: str, settings: dict | None = None) -> dict:
    template = get_label_template(template_key)
    if not template:
        return {}
    effective_settings = load_label_settings() if settings is None else normalize_label_settings(settings)
    entry = effective_settings.get(template.get("key")) or {}
    dimensions = clean_label_dimensions(entry.get("dimensions"))
    if "width_mm" in dimensions:
        template["width_mm"] = dimensions["width_mm"]
    if "height_mm" in dimensions:
        template["height_mm"] = dimensions["height_mm"]
    return template


def processing_param_template_options(label_key: str) -> list[dict]:
    normalized_label_key = str(label_key or "").strip()
    default_template_key = PROCESSING_PARAM_TEMPLATE_BINDINGS_DEFAULT.get(normalized_label_key, "")
    default_template = LABEL_TEMPLATE_MAP.get(default_template_key) or {}
    size_key = str(default_template.get("size_key") or normalized_label_key).strip()
    mode = str(default_template.get("mode") or ("cz" if normalized_label_key == "item_cz" else "no-cz")).strip()
    options: list[dict] = []
    for template in LABEL_TEMPLATES:
        if template.get("category") != "product":
            continue
        if str(template.get("size_key") or "").strip() != size_key:
            continue
        if str(template.get("mode") or "").strip() != mode:
            continue
        options.append(dict(template))
    return options


def normalize_processing_param_template_bindings(data: dict | None) -> dict[str, str]:
    source = data if isinstance(data, dict) else {}
    normalized: dict[str, str] = {}
    for label_key, default_template_key in PROCESSING_PARAM_TEMPLATE_BINDINGS_DEFAULT.items():
        allowed_template_keys = {
            str(item.get("key") or "").strip()
            for item in processing_param_template_options(label_key)
            if str(item.get("key") or "").strip()
        }
        template_key = str(source.get(label_key) or "").strip()
        if template_key not in allowed_template_keys:
            template_key = default_template_key
        normalized[label_key] = template_key
    return normalized


def resolve_processing_param_template_key(label_key: str, bindings: dict | None = None) -> str:
    normalized_label_key = str(label_key or "").strip()
    if normalized_label_key not in PROCESSING_PARAM_TEMPLATE_BINDINGS_DEFAULT:
        return normalized_label_key
    normalized_bindings = normalize_processing_param_template_bindings(bindings)
    return normalized_bindings.get(
        normalized_label_key,
        PROCESSING_PARAM_TEMPLATE_BINDINGS_DEFAULT[normalized_label_key],
    )


def _load_label_settings_payload() -> dict:
...
def _extract_template_settings_payload(data: dict | None) -> dict:
    if not isinstance(data, dict):
        return {}
...
def _extract_processing_template_bindings_payload(data: dict | None) -> dict:
    if not isinstance(data, dict):
        return {}
    bindings_payload = data.get("processing_param_templates")
    return bindings_payload if isinstance(bindings_payload, dict) else {}


def normalize_label_settings(data: dict | None) -> dict:
    source = _extract_template_settings_payload(data)
    if not isinstance(source, dict):
        return {}
    normalized = {}
    for key in LABEL_TEMPLATE_KEYS:
        ...
        dimensions = clean_label_dimensions(entry.get("dimensions"))
        if not text and not fonts and not enabled and not dimensions:
            continue
        normalized[key] = {"text": text, "fonts": fonts}
        if enabled:
            normalized[key]["enabled"] = enabled
        if dimensions:
            normalized[key]["dimensions"] = dimensions
    return normalized


def synchronize_product_label_settings(
    data: dict | None,
    *,
    ...
    for regular_key, honest_sign_key in PRODUCT_LABEL_TEMPLATE_PAIRS:
        if normalized_source_key == honest_sign_key:
            authority_key = honest_sign_key
        elif normalized_source_key == regular_key:
            authority_key = regular_key
        elif regular_key in normalized:
            authority_key = regular_key
        elif honest_sign_key in normalized:
            authority_key = honest_sign_key
        else:
            continue
        authority = normalized.get(authority_key) or {}
        for target_key in (regular_key, honest_sign_key):
            target = normalized.get(target_key) or {"text": {}, "fonts": {}}
            for section in ("text", "fonts", "enabled"):
                source_values = authority.get(section) or {}
                target_values = dict(target.get(section) or {})
                for field in PRODUCT_LABEL_SHARED_FIELDS:
                    if field in source_values:
                        target_values[field] = source_values[field]
                    else:
                        target_values.pop(field, None)
                if target_values:
                    target[section] = target_values
...
def load_label_settings() -> dict:
    return synchronize_product_label_settings(_load_label_settings_payload())


def load_processing_param_template_bindings() -> dict[str, str]:
    payload = _load_label_settings_payload()
    bindings_payload = _extract_processing_template_bindings_payload(payload)
    return normalize_processing_param_template_bindings(bindings_payload)


def _save_label_settings_payload(*, template_settings: dict | None = None, bindings: dict | None = None) -> None:
    path = label_settings_path()
    normalized_templates = normalize_label_settings(template_settings)
    normalized_bindings = normalize_processing_param_template_bindings(bindings)
    payload = {"templates": normalized_templates}
    if normalized_bindings:
        payload["processing_param_templates"] = normalized_bindings
    path.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")


def save_label_settings(data: dict) -> None:
    existing_bindings = load_processing_param_template_bindings()
    _save_label_settings_payload(
        template_settings=synchronize_product_label_settings(data),
        bindings=existing_bindings,
    )


def save_processing_param_template_bindings(data: dict | None) -> None:
    existing_templates = load_label_settings()
    _save_label_settings_payload(template_settings=existing_templates, bindings=data)
