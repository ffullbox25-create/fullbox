...
class MoveRequest(models.Model):
    ...
    PROCESS_CHOICES = [
        (PROCESS_SHIPPING, "Отгрузка"),
        (PROCESS_FBS, "Перемещение ФБС"),
        (PROCESS_PROCESSING, "Обработка"),
        (PROCESS_RECEIVING, "Приёмка"),
        (PROCESS_MANUAL, "Ручной запрос"),
    ]
    ...
    # Empty means "not resolved": readers must fall back to the legacy heuristic.
...
_FBS_CONTEXT_PREFIXES = ("fbs-movement:", "fbs-floor-movement:")
_FBS_PAYLOAD_KEYS = (
    "fbs_movement_number",
    "fbs_movement_id",
    "fbs_replenishment_bridge_v1",
)
_SHIPPING_PAYLOAD_KEYS = (
    "shipping_order_id",
    "shipping_order_pk",
...
def resolve_move_request_process(
    *,
    context_type: str = "",
    context_id: str = "",
    ...
    """Name the warehouse process that ordered this reach-truck work.

    Pure function: no database access, no service imports.  The same rules are
    duplicated verbatim inside migration 0004; the duplication is deliberate so
    the data migration survives future edits to this module.  Rules are ordered
    ...
    if context_id_key.startswith(_FBS_CONTEXT_PREFIXES) or any(
        values.get(key) for key in _FBS_PAYLOAD_KEYS
    ):
        return MoveRequest.PROCESS_FBS
    if context_type_key == "processing" or has_value("processing_order_id"):
        return MoveRequest.PROCESS_PROCESSING
    # Shipping is checked before receiving on purpose.  A pick for shipping keeps
    # ``receiving_order_id`` in its payload as provenance of the source pallet,
    # and reading that as "ordered by receiving" mislabels almost every pick.
    if any(has_value(key) for key in _SHIPPING_PAYLOAD_KEYS) or destination_key == "OTG":
        return MoveRequest.PROCESS_SHIPPING
    if context_type_key == "receiving" or has_value("receiving_order_id"):
        return MoveRequest.PROCESS_RECEIVING
    return MoveRequest.PROCESS_MANUAL
...
class BoxClaim(models.Model):
    ...
    agency = models.ForeignKey(
        Agency,
        on_delete=models.CASCADE,
        related_name="reachtruck_box_claims",
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["agency", "status", "box_code"], name="reachtruck__agency__61a8d9_idx"),
            models.Index(fields=["agency", "status", "pallet_code"], name="reachtruck__agency__1399e5_idx"),
            models.Index(fields=["move_task", "status"], name="reachtruck__move_ta_3b310e_idx"),
            models.Index(fields=["shipping_order_id", "status"], name="reachtruck__shippin_dde26c_idx"),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=["agency", "box_code"],
                condition=Q(status="claimed"),
                name="uniq_active_reachtruck_box_claim",
            ),
        ]
...
class PalletLock(models.Model):
    ...
    agency = models.ForeignKey(
        Agency,
        on_delete=models.CASCADE,
        related_name="reachtruck_pallet_locks",
    )
    move_task = models.ForeignKey(
        MoveTask,
        on_delete=models.CASCADE,
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["agency", "status", "pallet_code"], name="reachtruck__agency__715fd2_idx"),
            models.Index(fields=["move_task", "status"], name="reachtruck__move_ta_fa7db5_idx"),
        ]
        constraints = [
            models.UniqueConstraint(
                fields=["agency", "pallet_code"],
                condition=Q(status="active"),
                name="uniq_active_reachtruck_pallet_lock",
            ),
        ]
