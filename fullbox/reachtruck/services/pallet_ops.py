...
BOX_SELECTION_FIXED = "fixed"
BOX_SELECTION_ANY_MATCHING = "any_matching"
BOX_SELECTION_PATTERN_MATCHING = "pattern_matching"
...
def _matched_requested_item_barcode(
    item,
    ...
    *,
    ...
    normalized_barcodes = {
        ...
        for value in (barcode_values or set())
    ...
    if item_barcode:
        return item_barcode if item_barcode.casefold() in normalized_barcodes else ""
    sku_code = str(item.get("sku") or item.get("sku_code") or "").strip()
    if not agency_id or not sku_code:
        return ""
    client_barcodes = SKUBarcode.objects.filter(
    ...
    return next(
        (
            str(value or "").strip()
            for value in client_barcodes
            if str(value or "").strip().casefold() in normalized_barcodes
        ),
        "",
    )
...
def _item_matches(
    ...
    *,
    ...
    if normalized_barcodes:
        return bool(
            _matched_requested_item_barcode(
                item,
                barcode_values,
                agency_id=agency_id,
            )
...
def _casefold_barcode_qty_map(raw) -> dict[str, int]:
    """Normalize barcode identity for comparisons without changing stored values."""

    result: dict[str, int] = {}
    for barcode, qty in _normalize_barcode_qty_map(raw).items():
...
def _normalize_requested_box_patterns(raw) -> list[dict]:
    source = raw
    if isinstance(raw, str):
        text = raw.strip()
        if not text:
    ...
    for item in source:
        if not isinstance(item, dict):
            continue
        barcode_qty = _normalize_barcode_qty_map(
            item.get("barcode_qty")
            or item.get("requested_barcode_qty")
            or item.get("requested_barcode_qty_json")
        )
        box_qty = _parse_int_value(item.get("box_qty") or item.get("qty") or item.get("requested_qty"))
        if box_qty <= 0 and barcode_qty:
            box_qty = _barcode_qty_total(barcode_qty)
        requested_box_count = _parse_int_value(
            item.get("requested_box_count") or item.get("count") or item.get("boxes")
        )
        if requested_box_count <= 0:
            requested_box_count = 1
        requested_article = str(item.get("requested_article") or item.get("sku_code") or "").strip()
        requested_goods_type = _normalize_goods_type(
            item.get("requested_goods_type") or item.get("goods_type")
        )
        barcodes_raw = item.get("requested_barcodes")
        ...
        if barcode_qty and not requested_barcodes:
            requested_barcodes = list(barcode_qty.keys())
        if box_qty <= 0:
            continue
        key = (
            box_qty,
        ...
        if key not in merged:
            ...
            order.append(key)
        merged[key]["requested_box_count"] = (
            _parse_int_value(merged[key].get("requested_box_count")) + requested_box_count
        )
    return [dict(merged[key]) for key in order if merged.get(key)]


def _requested_box_patterns(payload: dict) -> list[dict]:
    if not isinstance(payload, dict):
        return []
    if payload.get("requested_box_patterns") is not None:
        return _normalize_requested_box_patterns(payload.get("requested_box_patterns"))
    if payload.get("requested_box_patterns_json") is not None:
        return _normalize_requested_box_patterns(payload.get("requested_box_patterns_json"))
    return []
...
def _normalize_partial_pick_patterns(raw) -> list[dict]:
    ...
    for item in source:
        ...
        pick_barcode_qty = _normalize_barcode_qty_map(item.get("barcode_qty") or item.get("pick_barcode_qty"))
        source_qty = _parse_int_value(item.get("source_box_qty") or item.get("box_qty"))
        pick_qty = _parse_int_value(item.get("pick_qty") or item.get("qty") or item.get("requested_qty"))
        if source_qty <= 0 and source_barcode_qty:
            source_qty = _barcode_qty_total(source_barcode_qty)
        if pick_qty <= 0 and pick_barcode_qty:
            pick_qty = _barcode_qty_total(pick_barcode_qty)
        count = _parse_int_value(item.get("requested_box_count") or item.get("count") or item.get("boxes"))
        if count <= 0:
            count = 1
        if source_qty <= 0 or pick_qty <= 0:
            continue
        requested_barcodes_raw = item.get("requested_barcodes")
        ...
        if source_barcode_qty and not requested_barcodes:
            requested_barcodes = list(source_barcode_qty.keys())
        result.append(
            {
...
def _expand_partial_pick_patterns(payload: dict) -> list[dict]:
    ...
    for pattern in _partial_pick_patterns(payload):
        requested_box_count = _parse_int_value(pattern.get("requested_box_count"))
        if requested_box_count <= 0:
            continue
        for _index in range(requested_box_count):
            expanded.append(dict(pattern))
    return expanded


def _stock_box_matches_partial_pick_pattern(row: dict, pattern: dict) -> bool:
    return _stock_box_matches_requested_pattern(
        row,
        {
            "box_qty": _parse_int_value(pattern.get("source_box_qty")),
...
def _assign_partial_pick_patterns_to_stock_boxes(
    payload: dict,
    pallet_code: str,
    *,
    agency_id: int | None = None,
    ...
    for pattern_idx, pattern in enumerate(patterns):
        matches = [
            ...
            if _stock_box_matches_partial_pick_pattern(box, pattern)
        ]
        if not matches:
            return []
    ...
    ordered_patterns = sorted(
        ...
        key=lambda idx: (
            ...
            -_parse_int_value(patterns[idx].get("source_box_qty")),
            idx,
        ),
    )
    ...
    if not backtrack(0):
        return []
    return [(boxes[assignment[idx]], patterns[idx]) for idx in range(len(patterns)) if idx in assignment]


def _expand_requested_box_patterns(patterns: list[dict]) -> list[dict]:
    expanded: list[dict] = []
    for pattern in _normalize_requested_box_patterns(patterns):
        requested_box_count = _parse_int_value(pattern.get("requested_box_count"))
        if requested_box_count <= 0:
            continue
        for _index in range(requested_box_count):
...
def _requested_box_selection_mode(payload: dict) -> str:
    if not isinstance(payload, dict):
        return BOX_SELECTION_FIXED
    token = str(payload.get("requested_box_selection") or "").strip().lower()
    if token == BOX_SELECTION_ANY_MATCHING:
        return BOX_SELECTION_ANY_MATCHING
    if token == BOX_SELECTION_PATTERN_MATCHING:
        return BOX_SELECTION_PATTERN_MATCHING
    return BOX_SELECTION_FIXED


def _requested_box_count(payload: dict) -> int:
    if not isinstance(payload, dict):
        return 0
    return max(_parse_int_value(payload.get("requested_box_count")), 0)
...
def _otg_selector_sets(
    ...
    *,
    ...
    if barcode_values:
        ...
        scoped_agency_id = agency_id or _parse_int_value(payload.get("agency_id"))
        if scoped_agency_id:
            barcode_queryset = barcode_queryset.filter(agency_id=scoped_agency_id)
        sku_values.update(
...
def _all_stock_boxes_for_pallet(
    ...
    *,
    ...
    cache_key = (int(agency_id) if agency_id else None, target_code)
    if stock_boxes_cache is not None and cache_key in stock_boxes_cache:
        boxes = stock_boxes_cache[cache_key]
    else:
        from sklad.services.stock_operations import OperationalStockService

        boxes = OperationalStockService.get_pallet_boxes(target_code, agency_id=agency_id)
        if stock_boxes_cache is not None:
            stock_boxes_cache[cache_key] = list(boxes or [])
    if not boxes:
        return []
    return [
        {
    ...
    *,
    ...
    for box in _all_stock_boxes_for_pallet(
        ...
        for item in box.get("items") or []:
            matched_barcode = ""
            if barcode_values:
                matched_barcode = _matched_requested_item_barcode(
                    item,
                    barcode_values,
                    agency_id=agency_id,
                )
            ...
            if qty <= 0:
                continue
            total += qty
            barcode = matched_barcode or _resolved_item_barcode(item)
            if barcode and (not barcode_values or barcode in barcode_values):
                barcode_qty[barcode] = barcode_qty.get(barcode, 0) + qty
        if total <= 0:
            continue
...
def _stock_box_matches_requested_pattern(row: dict, pattern: dict) -> bool:
    row_qty = _parse_int_value(row.get("qty"))
    row_barcode_qty = _normalize_barcode_qty_map(row.get("barcode_qty"))
    pattern_qty = _parse_int_value(pattern.get("box_qty"))
    pattern_barcode_qty = _normalize_barcode_qty_map(pattern.get("barcode_qty"))
    if pattern_qty > 0 and row_qty != pattern_qty:
        return False
    if pattern_barcode_qty:
        return _casefold_barcode_qty_map(row_barcode_qty) == _casefold_barcode_qty_map(pattern_barcode_qty)
    requested_barcodes = {
    ...
    if requested_barcodes:
        row_barcodes = {barcode.casefold() for barcode in row_barcode_qty.keys() if barcode}
        if row_barcodes != requested_barcodes:
            return False
    return row_qty > 0


def _matching_stock_boxes_for_patterns(
    pallet_code: str,
    patterns: list[dict],
    *,
    agency_id: int | None = None,
    ...
    normalized_patterns = _normalize_requested_box_patterns(patterns)
    ...
    for box in _all_stock_boxes_for_pallet(
        ...
        if not any(_stock_box_matches_requested_pattern(candidate, pattern) for pattern in normalized_patterns):
            continue
        seen.add(box_key)
...
def _box_items_signature(box: dict) -> tuple[tuple[str, str, str, int], ...]:
    ...
    for item in box.get("items") or []:
        ...
        signature.append(
            (
                str(item.get("sku") or item.get("sku_code") or "").strip(),
                _resolved_item_barcode(item),
                _normalize_goods_type(item.get("goods_type") or item.get("goods_type_label")),
                qty,
            )
        )
    signature.sort()
...
def _matching_full_box_pick_meta(
    payload: dict,
    pallet_code: str,
    *,
    agency_id: int | None = None,
) -> dict[str, object]:
    barcode_values, sku_values, goods_type_values, requested_qty = _otg_selector_sets(
        payload,
        agency_id=agency_id,
    )
    if requested_qty <= 0 or not (barcode_values or sku_values or goods_type_values):
        return {}
    ...
    for box in all_boxes:
        box_code = str(box.get("code") or "").strip()
        match_row = matching_by_code.get(box_code.lower())
        if not box_code or not match_row:
            continue
        full_box_qty = _parse_int_value(box.get("qty"))
        matched_qty = _parse_int_value(match_row.get("qty"))
        if full_box_qty <= 0 or matched_qty != full_box_qty:
            return {}
        candidate_boxes.append(box)
    if len(candidate_boxes) < 2:
        return {}

    per_box_qty = _parse_int_value(candidate_boxes[0].get("qty"))
    if per_box_qty <= 0 or requested_qty % per_box_qty != 0:
        return {}
    requested_box_count = requested_qty // per_box_qty
    if requested_box_count <= 0 or requested_box_count > len(candidate_boxes):
        return {}

    first_barcode_qty = _normalize_barcode_qty_map(candidate_boxes[0].get("barcode_qty"))
    first_signature = (
        per_box_qty,
        tuple(sorted(first_barcode_qty.items())),
        _box_items_signature(candidate_boxes[0]),
    )
    for candidate in candidate_boxes[1:]:
        barcode_qty = _normalize_barcode_qty_map(candidate.get("barcode_qty"))
        candidate_signature = (
            _parse_int_value(candidate.get("qty")),
            tuple(sorted(barcode_qty.items())),
            _box_items_signature(candidate),
        )
    ...
    requested_barcode_qty = _requested_barcode_qty(payload)
    if requested_barcode_qty:
        if set(requested_barcode_qty.keys()) != set(first_barcode_qty.keys()):
            return {}
        for barcode, per_box_value in first_barcode_qty.items():
            if _parse_int_value(requested_barcode_qty.get(barcode)) != _parse_int_value(per_box_value) * requested_box_count:
                return {}
...
def _resolve_stock_otg_box_codes(
    ...
    *,
    ...
    partial_assignments = _assign_partial_pick_patterns_to_stock_boxes(
        payload,
        pallet_code,
        agency_id=agency_id,
        stock_boxes_cache=stock_boxes_cache,
    )
    if _partial_pick_patterns(payload):
        if partial_assignments:
            return [
                str(box.get("code") or "").strip()
                for box, _pattern in partial_assignments
                if str(box.get("code") or "").strip()
            ], ""
        return [], "Не найден подходящий короб для частичного отбора."
...
def _resolve_stock_box_partial_codes(
    ...
    *,
    ...
    to_zone = str((payload.get("to_location") or {}).get("zone") or "").strip().upper()
    if to_zone == "OTG":
        return _resolve_stock_otg_box_codes(
            payload,
            pallet_code,
            agency_id=agency_id,
            stock_boxes_cache=stock_boxes_cache,
...
def _pallet_box_plan(
    ...
    del pallet_lookup
    all_boxes = _all_stock_boxes_for_pallet(
        pallet_code,
        agency_id=agency_id,
        stock_boxes_cache=stock_boxes_cache,
    )
    ...
    pattern_slots_by_code: dict[str, list[int]] = {}
    ...
    elif move_mode == MOVE_MODE_BOX_FULL and selection_mode in {
        BOX_SELECTION_ANY_MATCHING,
        BOX_SELECTION_PATTERN_MATCHING,
    }:
        if selection_mode == BOX_SELECTION_PATTERN_MATCHING:
            expanded_patterns = _expand_requested_box_patterns(_requested_box_patterns(payload))
            for box in all_boxes:
                box_code = str(box.get("code") or "").strip()
                box_key = box_code.lower()
                if not box_code:
                ...
                slot_indexes = [
                    pattern_index
                    for pattern_index, pattern in enumerate(expanded_patterns)
                    if _stock_box_matches_requested_pattern(candidate, pattern)
                ]
                if not slot_indexes:
                    continue
                matching_codes.add(box_key)
                pattern_slots_by_code[box_key] = slot_indexes
        else:
            barcode_values, sku_values, goods_type_values, _requested_qty = _otg_selector_sets(
                payload,
                agency_id=agency_id,
            )
            matching_codes = {
                str(row.get("code") or "").strip().lower()
                for row in _matching_stock_boxes_for_pallet(
                    pallet_code,
                    barcode_values,
                    sku_values,
                    goods_type_values,
    ...
    for box in all_boxes:
        ...
        if len(barcode_qty) > 4:
            preview += "; ..."
        box_key = str(box.get("code") or "").strip().lower()
        if matching_codes and box_key in matching_codes:
            action = "Подходит"
        else:
            action = "Доставить" if box_key in deliver_codes else "Вернуть"
        planned.append(
            {
                ...
                "action": action,
                "is_matching": bool(matching_codes and box_key in matching_codes),
                "selection_pattern_slots": pattern_slots_by_code.get(box_key, []),
                "sort_order": 0 if action != "Вернуть" else 1,
            }
        )
...
def _box_execution_plan(
    ...
    if partial_patterns:
        ...
        for row in pallet_plan:
            if str(row.get("action") or "").strip() != "Доставить":
                continue
            ...
            for pattern_idx, pattern in enumerate(partial_patterns):
                if pattern_idx in used_patterns:
                    continue
                if not _stock_box_matches_partial_pick_pattern(candidate, pattern):
                    continue
                requested_by_box[box_key] = int(requested_by_box.get(box_key, 0)) + _parse_int_value(pattern.get("pick_qty"))
        ...
        plan_rows.append(
            {
                ...
                "remain_in_box_qty": remain_in_box_qty,
